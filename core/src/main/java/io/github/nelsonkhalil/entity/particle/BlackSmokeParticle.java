...
public class BlackSmokeParticle implements Particle {

    private static final float MAX_LIFE = 2F;
    private static final float MIN_LIFE = 1.4F;

    private final GeneralParticleInfo ginfo;
    private final BlackSmokeParticleInfo info;

    private final Sprite sprite;

    public BlackSmokeParticle(PartialGeneralParticleInfo ginfo, BlackSmokeParticleInfo info, AssetLoader al) {
        this.ginfo = ginfo.withRandomLife(MIN_LIFE, MAX_LIFE);
        this.info = info;
        this.sprite = new Sprite(al.getTexture(MultiFileTexture.BLACK_SMOKE.getRandom()));
    }
